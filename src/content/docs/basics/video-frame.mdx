...
Finally, `VideoFrame` objects have a `timestamp` (the time in the video's timeline at which the frame appears) and a `duration` which is the duration.

Again, remember that VideoFrame and other WebCodecs interfaces represent time in microseconds.
