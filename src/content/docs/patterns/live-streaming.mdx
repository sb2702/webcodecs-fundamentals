...
So, for livestreaming with WebCodecs from the server to many clients, Media over Quic is the necessary enabling network layer, and WebCodecs + Media over Quic as a stack has concerete advantages compared to WebRTC and HLS/DASH streaming (low latency and high volume).
...
In cases where your application owns the end broadaster -> viewer stack, Media over Quic + WebCodecs makes much more sense as there are concrete performance and costs benefits compared to older workflows, and in that scenario you might not even need a server to stream video.
...
You can use either a CLI or a Rust/JS library to generate a key. Here's how you'd install the CLI (Rust is a prequisite)
