...
The `@moq/lite` client we used earlier in the browser also works on the server, although as of January 2026, WebTransport is not available in any server js environment like Node, Bun or Deno, so the server will fall back to connecting to the relay via WebSockets.

Additionally, to use `@moq/lite` in Node, you'll need to use a polyfill to enable WebSocket support
...
![](/assets/patterns/livestreaming/moq-server-demo.png)
...
Coming soon - server streaming via media over quic
