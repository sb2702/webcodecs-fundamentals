...
The next obvious step is to send encoded video over the network. The way to send arbitrary binary data over a track using the `@moq/lite` library is as follows:
...
```
Any subscriber can then listen for `catalog.json` in the broadcast, and once it receives the catelog, it can start listening for the video and audio tracks, as well as set up the `AudioDecoder` and `VideoDecoder`
