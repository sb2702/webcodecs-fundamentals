...
Given Streamyard's core focus on stability, using the established WebRTC protocol over a newer, more experimental setup like Media over Quic probably makes sense, however for the local recordings use case (recording higher-quality versions of the stream locally, uploading that in chunks), WebCodecs would have a clear-cut benefit.
