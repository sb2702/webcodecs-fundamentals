...
For a streaming, we'd need to send a bunch of these chunks, in real time, in order, over a network.

![](/assets/patterns/livestreaming/encoded-chunk-3.svg)

You could theoretically choose any of a number of ways to send this data from a browser to a server. Here are a few options:

###### HTTP Endpoint

You could just expose an HTTP endpoint on your server and `POST` every frame as data. Here's a simplified example where metadata is sent as a header
...
You'd be sending 30+ chunks per second, and also audio, and this is failure prone. You could make this work with optimizations, such as grouping chunks of frames together, but there are inherently more suiable options.


###### Web Sockets

Web Sockets enables you create a persistent connection with a server, and send enables you to send a stream of multiple messages of data. Here you could come up with your own binary encoding scheme to fit metadata and chunk data together into a single binary buffer, and send that over.
